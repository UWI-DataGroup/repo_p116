** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name          heart_duplicates.do
    //  project:                BNR
    //  analysts:               Jacqueline CAMPBELL
    //  date first created      31-MAY-2021
    // 	date last modified      09-MAR-2022
    //  algorithm task          Importing data from REDCap via API, removing non-heart data arms, identifying duplicates and exporting flagged duplicates back into REDCap
    //  status                  Completed
    //  objective               (1) To have list of possible heart duplicates identified during this process so DAs can correct in REDCap's BNRCVD_CORE database.
	...
	//							The possible duplicates are flagged using the variable [duplicate] which is used for filtering the REDCap report called 'Possible Duplicates (HEART)'.
...
export delimited using "heart_dup.csv", replace
br
...
*** ---- H E A R T  D U P L I C A T E S ----- ***

drop if redcap_event_name=="stroke_arm_1" | redcap_event_name=="tracking_arm_3" | redcap_event_name=="reviewing_arm_4" | redcap_event_name=="dashboards_arm_5"
...
sort lname fname cfadmdate record_id
list record_id fname lname natregno cfadmdate dupadmpt duplicate dupcheck if dupadmpt>0 & redcap_event_name=="heart_arm_2"
...
gen nameslist=1 if  (dupadmpt>0 & redcap_event_name=="heart_arm_2" & fname!="" & lname!="" & cfadmdate!="" & duplicate==.) ///
					   |(dupadmpt>0 & redcap_event_name=="heart_arm_2" & fname!="" & lname!="" & cfadmdate!="" & dupcheck==.) //flagging possible NAMES duplicates 
//23 changes
replace duplicate=3 if  (dupadmpt>0 & redcap_event_name=="heart_arm_2" & fname!="" & lname!="" & cfadmdate!="" & duplicate==.) ///
					   |(dupadmpt>0 & redcap_event_name=="heart_arm_2" & fname!="" & lname!="" & cfadmdate!="" & dupcheck==.) //flagging possible duplicates 
//23 changes
...
drop if duplicate!=3 //1152 deleted
local fileforimport "data_for_import_namesH.csv"
...
list record_id fname lname natregno cfadmdate dupadmnrn duplicate dupcheck redcap_event_name nameslist if dupadmnrn>0 & redcap_event_name=="heart_arm_2"

replace duplicate=3 if (dupadmnrn>0 & redcap_event_name=="heart_arm_2" & natregno!="." & cfadmdate!="" & duplicate==.) ///
					  |(dupadmnrn>0 & redcap_event_name=="heart_arm_2" & natregno!="." & cfadmdate!="" & dupcheck==.) //flagging possible duplicates
...
**Push data back into REDCap project
local fileforimport "data_for_import_nrnH.csv"
export delimited using `fileforimport', nolabel replace
//stop //delete once ready to push into REDCap
